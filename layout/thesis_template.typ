...
#let thesis(
  ...
  set page(
    margin: (left: 30mm, right: 15mm, top: 30mm, bottom: 20mm),
    numbering: "1",
    number-align: center,
    header: grid(columns: 1fr, align: (center), v(5mm), title, v(2mm), grid.hline()),
  )
  ...
  show ref: it => {
    ...
    if el != none and el.func() == heading and el.level == 1 {
      link(
        el.location(),
        [Глава #numbering(el.numbering, ..counter(heading).at(el.location()))],
      )
    } else {
      it
    }
  ...
  bibliography("../thesis.bib", full: true, title: "Список Литературы", style: "ieee")
}
